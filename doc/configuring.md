...
Copy the private genomes to a new home that matches genome-grist requirements like so:
...
There will also be an "information file", `databases/podar-ref.info.csv`, that contains three columns. These were autogenerated by the script from the FASTA files you gave it. You can edit the `display_name` column and change it to whatever you want; the other columns need to match with other information so please don't change those!
