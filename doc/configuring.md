...
provide one FASTQ file per sample in the `trim/`
...
need to create `outputs.private/trim/podar.trim.fq.gz`.
...
* you can put your own sourmash signature (k=31, scaled=1000) in `sigs/{sample}.trim.sig` if you want to have it do the database search for you;
